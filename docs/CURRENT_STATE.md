...
Confirmado: `202609220024_production_batches.sql` falló al aplicarse a Supabase remoto (`supabase db push`) por dos bugs SQL reales, corregidos directamente en la misma migración (todavía no aplicada en ningún entorno, por lo que no se creó una migración `026`): `app_private.compute_production_preview` mezclaba `batch.cost_total_cents` (columna sin agregar) con `sum(shares.floor_cents)` sin `GROUP BY` (SQLSTATE 42803) en el CTE `leftover`, reemplazado por una subconsulta escalar; y tanto `compute_production_preview` como `get_production_catalog` reproducían el bug de precedencia de precio por sucursal ya corregido en `202609100004_fix_branch_price_precedence.sql` (faltaba `nulls last` al ordenar por `pp.branch_id = ...`, lo que hubiera preferido silenciosamente un precio global sobre uno de sucursal). Ninguno de los dos había sido detectado por la revisión manual previa a la primera versión de este sprint; siguen sin validarse contra Postgres real.
