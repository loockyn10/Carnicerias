...
- **Precio de venta manual, costo derivado — implementado 2026-09-22** (ver D-037/D-038, `docs/DECISIONS.md`): reemplaza el flujo de costo+markup→gross-up (D-006, superseded) como camino de escritura vigente. `product_prices` sigue siendo la única fuente del precio, reutilizada sin cambios de schema; ahora se escribe directo vía `set_product_price` (ya existía, sin usar por ninguna UI hasta ahora) y el nuevo `bulk_set_product_prices` (migración `202609220030`, edición masiva atómica, precio global). El costo (`product_costs`) se alimenta automático al finalizar un desposte (`complete_production_batch`, migración `202609220029`) o manual vía el nuevo `set_product_cost` para productos comprados ya terminados. `set_cash_discount` (nuevo) reemplaza `set_cash_discount_and_reprice` como camino de escritura: cambia el % de descuento sin repricear ningún producto. `save_product_pricing`/`calculate_product_price`/`set_cash_discount_and_reprice`/`product_pricing_settings` quedan intactos en la base (historial, D-005) pero sin uso desde la UI.
- Admin: `product-pricing-fields.tsx` pasó de pedir costo+margen a pedir precio de venta (editable) + mostrar costo estimado vigente/ganancia/rentabilidad (solo lectura, derivados). Nueva pestaña "Productos → Precios" con `bulk-price-editor.tsx`: planilla compacta (producto, categoría, tipo de venta, precio actual, nuevo precio), guarda sólo las filas modificadas.
- Desposte: `production_batch_outputs.output_weight_grams` (peso real producido) sigue siendo obligatorio para **todo** output, sea `WEIGHT` o `UNIT` — un output `UNIT` (p. ej. 2 arrollados = 2,640 kg) sigue siendo materia física del lote y su peso cuenta para la merma/rendimiento, aunque **no** determine su valor comercial ni su costo asignado (eso usa cantidad × precio/unidad). `output_quantity_units` es adicional, obligatorio sólo cuando el producto vende por unidad. `packages/business-logic/src/production.ts` (`allocateProductionCost`) generalizado a un discriminated union WEIGHT/UNIT (ambas variantes cargan `outputWeightGrams`), sin cambiar el algoritmo de asignación por valor comercial (ya era agnóstico, sólo necesita el valor de venta en centavos). `products.approx_weight_grams` (opcional, informativo, nunca usado en ningún cálculo ni en el balance de merma de un lote — para eso siempre se usa el peso real de ese lote) para productos UNIT.
- Cálculos con cents enteros, basis points y redondeo half-up.
- **Corregido 2026-09-24 (ver D-044, sección propia más abajo)**: la regla de ajuste por medio de pago descripta acá (CASH/TRANSFER/OTHER con descuento, DEBIT/CREDIT sin ajuste) quedó invertida — el precio cargado ya es el precio de CASH/TRANSFER/OTHER sin ajuste, y DEBIT/CREDIT pagan ese precio más un recargo. El orden de pasos (lista → pago → promoción → final) no cambió, sólo la fórmula del paso 2.
- Promociones `PERCENTAGE` y `FIXED_PRICE_PER_KG` se aplican después del ajuste por medio de pago.
- Los ítems guardan snapshots separados de lista, costo, markup, descuento por pago, promoción y subtotal final.
- Productos legacy con precio vigente continúan vendiéndose sin inventar costo histórico. Un producto con precio pero sin costo todavía (nunca producido/comprado con costo cargado) es válido: se vende igual y muestra "costo no disponible".
- **No implementado este sprint (documentado, no improvisado)**: venta POS de productos `UNIT`. Bloqueado en tres capas independientes — filtro `unit_type==='WEIGHT'` del catálogo en `apps/pos/src/App.tsx`, structs Rust/SQLite sin campo de cantidad (`apps/pos/src-tauri/src/lib.rs`), y `complete_discounted_sale`/`sync_offline_sale` exigen `unit_type='WEIGHT'` en SQL. Sigue trackeado en `docs/TASKS.md`; el pricing/costo/desposte quedan listos para cuando se aborde.
- **REQUIERE VERIFICACIÓN**: mismo bloqueo que el resto del historial reciente — Docker Desktop no estuvo operativo en esta sesión, así que `202609220029`/`202609220030` se revisaron manualmente línea por línea pero no corrieron contra Postgres real; `database.types.ts`/`database.rpc-null-overrides.ts` se actualizaron a mano (mismo patrón ya usado). `pnpm typecheck`/`lint`/`test`/`build` sí corrieron limpios en todo el monorepo con estos cambios incluidos.

## Promociones pack, corrección de forma de venta y multicategoría — implementado 2026-09-23 (ronda 1)

- **Bug "producto nuevo no aparece en Promociones" — diagnosticado, no era un bug de filtro**: `apps/admin/src/app/admin/promotions/page.tsx` filtraba el selector a `unit_type==='WEIGHT'` porque `product_weight_discounts` sólo soportaba esa modalidad por diseño (el propio RPC `save_weight_discount` ya rechazaba productos `UNIT` con `unit_type='WEIGHT'` server-side). Un producto como "Hamburguesa" (naturalmente `UNIT`) quedaba invisible por eso, no por caché ni por un filtro mal escrito. Se verificó auditando el código (sin acceso a datos reales de producción en esta sesión — Docker no disponible, ver más abajo); si el producto reportado resultara ser `WEIGHT`+activo y aun así faltara, sería un bug distinto no cubierto por este cambio.
- **Nueva modalidad de promoción, `PACK_FIXED_TOTAL`** (migración `202609230031_promotion_pack_fixed_total.sql`, ver D-039 y `docs/DOMAIN_RULES.md`): una cantidad concreta a precio TOTAL fijo (ej. "Vacío 2kg por $18.000", "Hamburguesa 40u por $28.000"), no un umbral. `product_weight_discounts` gana `promotion_mode`, `pack_quantity_grams`/`pack_quantity_units`/`pack_price_cents`; `minimum_grams`/`discount_type`/`discount_value` pasan a nullable (sin tocar el comportamiento `THRESHOLD` existente). Disponible para `WEIGHT` y `UNIT` — esto es lo que realmente resuelve el bug de arriba. `save_weight_discount` extendido con parámetros opcionales al final (misma técnica de compatibilidad ya usada en el repo). Índice único nuevo: máximo un pack activo por producto/sucursal.
- **POS conecta el pack WEIGHT al flujo de venta real**: `complete_discounted_sale`/`sync_offline_sale` (mismo archivo de migración) validan una línea pack tanto online como offline; guarda de cordura: el precio del pack no puede superar el precio de lista para el peso realmente pesado. `apps/pos/src/App.tsx` ofrece un toggle "Vender como pack" cuando corresponde; `apps/pos/src-tauri/src/lib.rs` (`insert_sale`) re-valida el pack contra `local_weight_discounts` antes de persistir localmente — cubierto por 2 tests Rust nuevos que sí corrieron contra SQLite real. El pack **UNIT** quedó modelado y configurable en Admin en esta ronda, con su fórmula de múltiplos exactos + resto a precio normal probada en `packages/business-logic` (`calculateUnitPackSalePricing`); el POS todavía no vendía productos `UNIT` en absoluto al cierre de esta ronda (resuelto en la ronda 2, ver más abajo).
- **Corrección de forma de venta (WEIGHT ↔ UNIT)** (migración `202609230032_product_unit_type_change_guard.sql`, ver D-040): `save_product` ya aceptaba este cambio en su rama UPDATE sin ninguna protección — lo único que lo impedía era que `ProductManageModal` mandaba `unit_type` oculto. Ahora bloquea el cambio si el producto tiene cualquier fila en `sale_items`, `stock_movements`, `production_batch_outputs`/`production_batches`, o `product_weight_discounts` (activa o no); precio/costo no bloquean. `ProductManageModal` expone el selector (deshabilitado cuando `get_products_with_unit_type_history` marca el producto).
- **Multicategoría** (migración `202609230033_product_multi_category.sql`, ver D-041): `product_category_assignments` + `set_product_categories` (reconcilia principal + adicionales atómicamente, agrega la principal al set si faltara en vez de rechazar). `pull_pos_state` y `get_pos_catalog` (recreada vía DROP+CREATE, ganó una columna de retorno) exponen `categoryIds`; SQLite gana `catalog_product_categories` (migración `008`). El filtro de categoría del POS (`apps/pos/src/lib/catalog.ts`, con tests) considera todas las categorías asignadas; "Todos" no duplica. Limitación de esta ronda (resuelta en la ronda 2, ver D-043): el nombre/color de un tab salía sólo de la categoría principal de algún producto — una categoría que nunca era principal de nadie no tenía tab propio.
- **Buscadores** en `Productos → Precios` (`bulk-price-editor.tsx`) y `Productos → Promociones` (`promotions-list.tsx`, nuevo componente cliente): ambos cliente-side sobre datos ya cargados, sin roundtrip adicional, usando el helper compartido `apps/admin/src/lib/text-search.ts` (extraído de `branch-stock.ts`, que ya lo usaba para `/admin/branch-stock`).

## Venta POS `UNIT` end-to-end y directorio de categorías del POS — implementado 2026-09-23 (ronda 2)

Continuación del mismo día: las migraciones `202609230031`–`202609230033` de la ronda 1 todavía no estaban aplicadas a ningún entorno, así que esta ronda **editó esos archivos directamente** en vez de parchearlos con una migración nueva (`202609230033` ganó el RPC `get_pos_categories` y la clave `categories` en `pull_pos_state`); sólo la venta `UNIT` en sí, por ser alcance genuinamente nuevo, se agregó como migración propia.

- **Venta `UNIT` completa en el POS** (D-042): `apps/pos/src/App.tsx` dejó de filtrar el catálogo a sólo `WEIGHT` (tanto en la carga offline vía `localDatabase.catalog` como online vía `get_pos_catalog`). Tocar un producto `UNIT` abre un stepper de cantidad entera (`[-] 1 [+]` + input manual, arranca en 1), nunca la balanza ni un input de gramos. El pack `UNIT` (D-039) se aplica automáticamente por múltiplos exactos, sin toggle (a diferencia del pack `WEIGHT`, que sigue requiriendo confirmación explícita). Motor de precio reutilizado sin duplicar: `computeWeightLine`/`computeUnitLine` (funciones puras nuevas en `App.tsx`) llaman a las mismas `calculateSalePricing`/`calculateUnitPackSalePricing` de `packages/business-logic` que ya usaba `WEIGHT`, aplicando el mismo orden lista → descuento por medio de pago → promoción → final (D-008). "Modificar cantidad" reemplaza a "Modificar peso" para una línea `UNIT`, editando en el lugar sin sacar/reponer la línea. El ticket respeta la regla existente de "sin montos antes de elegir medio de pago" igual para `UNIT` que para `WEIGHT` (sin cambios en esa lógica).
- **Ejemplos exactos verificados** (business-logic + Rust, mismos números que pidió el negocio): 40 hamburguesas → 1 pack ($28.000); 80 → 2 packs ($56.000); 45 → 1 pack + 5 normales = $28.000 + $4.000 = **$32.000**; 39 → sin pack, $800×39 = $31.200.
- **Stock**: `sale_items.quantity_units` (columna nueva, separada de `weight_grams`, `CHECK` exige exactamente una de las dos) y `stock_movements.quantity_grams` reutilizado como contador de unidades con signo (mismo precedente que `PRODUCTION_YIELD`, D-038) — nunca se convierte a gramos, `approx_weight_grams` nunca interviene.
- **Funciona online y offline igual**: Postgres (`complete_discounted_sale`/`sync_offline_sale`, migración `202609230034_unit_sale_support.sql`) y Rust/SQLite (`insert_sale`, migración `009_unit_sale_support.sql`, outbox, `sync_offline_sale` desde el cliente, reintento/backoff existente, recuperación tras reinicio) implementan la misma validación replicada, no una ruta "sólo online".
- **Bug real encontrado y corregido antes de llegar al usuario**: cuatro RPCs preexistentes de sprints anteriores (`get_profitability_analytics`, `get_replenishment_plan`, `cancel_sale`, `get_admin_dashboard`) ya asumían que `sale_items.weight_grams` podía contener la cantidad de una venta `UNIT` — separar `quantity_units` en su propia columna las habría roto. La más grave: `cancel_sale` habría crasheado con una violación NOT NULL en `stock_movements.quantity_grams` al anular cualquier venta con una línea `UNIT`. Las cuatro se reemplazaron (`CREATE OR REPLACE FUNCTION`, misma firma) en la misma migración `202609230034`, usando `coalesce(weight_grams, quantity_units)`. Limitación cosmética aceptada: los widgets "top productos por kg" del dashboard siguen etiquetando en kg lo que para un producto `UNIT` es su cantidad; no afecta ranking por ingresos ni rentabilidad.
- **Bug real encontrado y corregido en el propio desarrollo (Rust/SQLite)**: la migración `009` original ponía `pragma foreign_keys = off/on` dentro del propio archivo SQL, ejecutado vía `transaction.execute_batch(...)`. Un test nuevo (`unit_sale_migration_preserves_preexisting_local_sale_history`, que aplica migraciones 1–8, inserta datos de venta reales con el schema viejo, y luego corre `initialize_connection`) detectó que esto no funcionaba: SQLite ignora `pragma foreign_keys` dentro de una transacción abierta, así que el `DROP TABLE`/rebuild de `local_sales`/`local_sale_items` corría con FK real activa y fallaba (`FOREIGN KEY constraint failed`). Se movió el toggle a Rust, fuera de la transacción, bracketing sólo la migración 9. Confirmado: el test pasa y preserva el historial de ventas ya confirmado localmente.
- **Directorio de categorías del POS, ya no inferido de productos** (D-043): `get_pos_categories(p_branch_id)` (nuevo RPC en `202609230033`) y tabla SQLite `catalog_categories` (migración `008`, ya existía la tabla; esta ronda corrige cómo se sincroniza) exponen id/nombre/color/orden de cada categoría con al menos una asignación real, independiente de cuál producto sea su principal. `apply_catalog_pull` (Rust) reconcilia marcando todo inactivo y dando de alta el set fresco (nunca `DELETE`, evita romper la FK de un producto local desactivado que aún referencia una categoría vieja). `apps/pos/src/lib/catalog.ts` (`buildCategoryTabs`) construye las tabs desde este directorio, no desde `products`. Caso concreto verificado: "Chorizo de cerdo" (principal Cerdo, secundaria Embutidos) aparece bajo ambas tabs y una sola vez en "Todos"; "Embutidos" genera su propia tab aunque ningún producto la tenga como principal.
- **REQUIERE VERIFICACIÓN (ambas rondas)**: Docker Desktop no estuvo operativo en ninguna sesión de este sprint, así que las migraciones `202609230031`–`202609230034` y los tests pgTAP (`promotions_pack.test.sql`, `product_unit_type_guard.test.sql`, `product_multi_category.test.sql`, `unit_sale_support.test.sql`) se revisaron manualmente línea por línea pero **no corrieron** contra Postgres real. `packages/database/src/database.types.ts` se actualizó a mano (mismo patrón que sesiones previas).

## Recargo por tarjeta (D-044) — implementado 2026-09-24

Corrige una regla de negocio que estaba invertida: el precio cargado en Productos ya es el precio de CASH/TRANSFER/OTHER (sin ningún ajuste); DEBIT/CREDIT ("Tarjeta") pagan ese precio más un recargo porcentual configurado en Admin. Ejemplo: $10.000 + 10% → CASH/TRANSFER = $10.000, DEBIT/CREDIT = $11.000.

- **Causa por la que Tarjeta no se diferenciaba**: el motor de precio (`calculateSalePricing`, `packages/business-logic/src/pricing.ts`) aplicaba el % configurado como **descuento** sólo para CASH/TRANSFER/OTHER (`listPrice*(1-bps)`), dejando DEBIT/CREDIT en el precio de lista sin cambios (`isDiscountEligiblePaymentMethod`). Esa dirección era exactamente la contraria a la regla real del negocio: el precio cargado debía ser el de efectivo, no un "precio de lista" del que efectivo se descuenta. `isDiscountEligiblePaymentMethod` se reemplazó por `isCardSurchargePaymentMethod` (misma partición de métodos, invertida en uso: ahora gatea quién recibe el recargo, no quién recibe el descuento) y la fórmula pasó de restar a sumar para DEBIT/CREDIT.
- **Bug adicional encontrado en la auditoría (no la causa raíz, pero sí bloqueante una vez que la migración `202609230031` se aplique)**: `get_pos_commercial_config` había perdido la clave `cashDiscountBps` del jsonb que devuelve desde el `CREATE OR REPLACE` de esa migración (agregó campos de pack a `discounts` pero omitió reconstruir esa clave) — el POS nunca recibía el porcentaje configurado. Corregido en la misma migración nueva de este sprint.
- **Capas**: `packages/business-logic/src/pricing.ts` (las tres funciones de pricing ganan `cardSurchargeCents`; orden lista→promoción/pack→recargo, con el recargo aplicado al total comercial completo sin excepción de pack). `packages/types`/`packages/sync` (`cardSurchargeCents` nuevo, opcional, en `TicketLine`/`OfflineSaleItemPayload`). `apps/pos/src/App.tsx` (ticket y modal de vista previa muestran "Recargo tarjeta" en ámbar junto al ya existente "Descuento por pago", que en la práctica queda en 0 para toda venta nueva — el recargo se muestra también en una línea pack, no sólo en líneas normales). `apps/pos/src-tauri/src/lib.rs` (`insert_sale` revalida `card_surcharge_cents` con la misma fórmula en los 4 casos WEIGHT/UNIT × pack/normal; `payment_method_receives_discount` se dejó intacto, sólo se invirtió cómo se usa su resultado). Admin (`pricing-settings-modal.tsx`: "RECARGO POR TARJETA" en vez de "DESCUENTO POR PAGO"; `set_cash_discount`'s mensaje de error actualizado).
- **Online y offline**: Postgres (`complete_discounted_sale`/`sync_offline_sale`, migración nueva `202609240035_card_surcharge_pricing.sql`) y Rust/SQLite (`insert_sale`, migración SQLite `010_card_surcharge_pricing.sql`) implementan la misma fórmula — mismo orden lista→promoción/pack→recargo, y el mismo redondeo en dos pasos (por unidad primero, subtotal después, para líneas normales; un único redondeo sobre el total, para líneas pack) — el offline puede rechazar un total manipulado igual que antes.
- **Nombres físicos conservados, sin migración destructiva de renombrado** (decisión explícita del pedido): `cash_discount_bps`/`cash_discount_cents` (Postgres y SQLite) mantienen sus nombres; ahora `cash_discount_bps` configura el recargo, y `cash_discount_cents` queda siempre en 0 para una venta nueva pero se conserva sin tocar en cada fila histórica ya grabada (D-005/D-009). El concepto nuevo vive en una columna propia: `sale_items.card_surcharge_cents`/`local_sale_items.card_surcharge_cents`.
- **Por qué se creó una migración nueva y no se editó `202609230031`/`202609230034`**: a diferencia de rondas anteriores de este mismo sprint (donde esas migraciones seguían sin aplicarse en ningún lado), la tarea inmediatamente anterior a esta confirmó evidencia directa de que al menos parte de ellas ya están aplicadas en un entorno real del usuario (dos overloads de `save_weight_discount` coexistiendo en producción). Por eso este cambio se agregó como incremental nueva (`202609240035`), no como edición de las anteriores.
- **PACK_FIXED_TOTAL: corrección del mismo día, sin excepción al recargo (ver D-044)**: la primera implementación dejaba el total de un pack invariante al medio de pago (mismo criterio conservador que D-039 usó para la ambigüedad de la promoción). El usuario corrigió esto explícitamente el mismo día: "no hay excepción para promociones ni packs". El total de un pack sigue siendo invariante al peso/cantidad real, pero el recargo por tarjeta se aplica sobre su total completo sin excepción — ejemplo obligatorio verificado: "Vacío 2kg/$18.000" con tarjeta = $19.800; "45 hamburguesas" (1 pack de 40 a $28.000 + 5 sueltas, $32.000 en efectivo) con tarjeta = $35.200 (recargo sobre el total completo, no sólo sobre las 5 sueltas). La migración `202609240035` y el Rust/tests correspondientes se corrigieron en el lugar (seguían sin aplicarse a ningún entorno).
- **Bug real encontrado y corregido durante esta misma corrección (App.tsx)**: el efecto que recalcula el ticket al cambiar de método de pago reusaba `line.subtotalCents` como `packPriceCents` al recalcular una línea pack WEIGHT — válido cuando el pack era invariante al medio de pago (ronda anterior), pero incorrecto ahora: ese subtotal puede venir ya recargado de un cálculo previo con otro método de pago, componiendo el recargo en cada cambio de método. Corregido para volver a buscar el precio real del pack en la regla vigente (`discounts`), igual que ya hacía la rama UNIT.
- **REQUIERE VERIFICACIÓN**: Docker Desktop no estuvo operativo en esta sesión; `202609240035_card_surcharge_pricing.sql` y el pgTAP nuevo (`supabase/tests/card_surcharge_pricing.test.sql`, 32 aserciones) se revisaron manualmente línea por línea pero **no corrieron** contra Postgres real. Sí corrieron contra un motor real: `cargo test` (Rust/SQLite, 39/39 OK, incluidos 8 tests nuevos/corregidos para la regla definitiva) y `pnpm check` (typecheck+lint+test) en todo el monorepo, además de `pnpm --filter @carnicerias/pos build` y `pnpm --filter @carnicerias/admin build`, todos verdes. `packages/database/src/database.types.ts` se actualizó a mano.
...
## Navegación Admin (sidebar + tabs) — implementado 2026-09-22

Sprint de UX/navegación puro: sin cambios de reglas de negocio, queries (salvo lectura de `?tab=` para resaltar pestaña), RLS, pricing, stock, ventas, empleados, POS, sync, desposte ni distribución.

- Sidebar (`components/admin-sidebar.tsx`) reducido a 9 entradas agrupadas: Principal (Inicio, Sucursales), Operación (Ventas, Stock, Desposte, Distribución), Gestión (Productos, Empleados), Sistema (Configuración). Sin submenús desplegables. El estado activo de cada entrada matchea por prefijo de ruta contra un array `match` (p. ej. "Ventas" queda activo en `/admin/sales`, `/admin/settlements` y `/admin/analytics`), no sólo por igualdad exacta.
- Nuevo componente reusable `components/section-tabs.tsx`: pestañas de subsección encima del contenido de una página, mismo patrón visual que `components/branch-tabs.tsx` (preexistente). Ninguna ruta se movió; las pestañas son links a las rutas ya existentes.
  - Ventas: `/admin/sales` (Historial) · `/admin/settlements` (Rendiciones) · `/admin/analytics` (Rentabilidad).
  - Stock: `/admin/stock` (Operaciones) · `/admin/branch-stock` (Por sucursal) · `/admin/replenishment` (Reposición).
  - Productos: `/admin/products` (Productos) · `/admin/products?tab=pricing` (Precios, mismo modal `PricingSettingsModal` ya existente, reubicado del header a esta pestaña) · `/admin/promotions` (Promociones).
  - Empleados: `/admin/employees` (Empleados) · `/admin/timekeeping` (Horas trabajadas). `/admin/timekeeping` no tenía título ni breadcrumb propio antes de este sprint; se agregó uno consistente con el resto de Admin.
- Nueva ruta `/admin/settings` (Configuración): página simple con cards agrupadas "Sistema" (Dispositivos POS, Avisos) y "Avanzado" (Auditoría); sin lógica propia más allá de `requireAdminContext()`. Dispositivos, Avisos y Auditoría conservan sus rutas (`/admin/devices`, `/admin/announcements`, `/admin/audit`) sin cambios funcionales.
- Detalle de sucursal (`components/branch-detail.tsx`, `components/branch-tabs.tsx`): eliminada la pestaña "Operación", que funcionaba como un segundo menú duplicado (links a Stock/Productos/Promociones/Avisos/Empleados/Dispositivos/Auditoría, ya accesibles por el sidebar). Quedan sólo `[Resumen] [Stock] [Ventas]`. El contenido real que vivía ahí (no navegación duplicada) se conservó, reubicado: "Editar datos" (`BranchForm`) y "Estado de la sucursal" (`BranchLifecyclePanel`, activar/desactivar/eliminar) pasaron a la pestaña Resumen; "Mermas recientes" y "Reingresos recientes" pasaron a la pestaña Stock. Ninguna función se eliminó.
- **Discrepancia detectada (regla de reconciliación)**: el pedido de este sprint asumía que la gestión administrativa de sucursales (crear/editar/desactivar) "todavía no existe" y pedía no inventarla, dejando a lo sumo un placeholder conceptual en Configuración. En realidad ya existe desde el sprint anterior (ver "Gestión de sucursales y pre-production reset" más abajo): `/admin/branches/new` y el `BranchForm`/`BranchLifecyclePanel` reubicados arriba. No se agregó ningún acceso a "Gestión de sucursales" dentro de Configuración para no duplicar navegación ni crear un placeholder redundante con lo que ya existe en Sucursales; la funcionalidad real sigue viviendo ahí, donde ya funcionaba.
- Sin cambios de performance: el sidebar conserva `prefetch={false}` tal como estaba (no se tocó, ver sección "Performance Admin"); `components/section-tabs.tsx` usa sólo `usePathname()` (mismo patrón que `branch-tabs.tsx`), sin `useSearchParams()`, para no requerir un límite `Suspense` adicional. La única pestaña que distingue por query (`/admin/products?tab=pricing`) resuelve el estado activo server-side (la propia página ya lee `searchParams`) y lo pasa como prop, sin hooks de cliente extra.
- Validación: `pnpm typecheck` / `pnpm lint` / `pnpm test` / `pnpm build` de `@carnicerias/admin` OK. **REQUIERE VERIFICACIÓN VISUAL**: Docker Desktop no estuvo operativo en esta sesión (mismo síntoma que sesiones anteriores) y no se dispone de credenciales de una cuenta admin real para iniciar sesión contra el proyecto Supabase remoto configurado en `apps/admin/.env.local`; no se intentó adivinar ni forzar credenciales. Se confirmó que `pnpm build` compila y que `/login` (única pantalla alcanzable sin sesión) renderiza con `pnpm dev` sin errores de consola. Falta smoke manual autenticado: sidebar, Inicio, Sucursales, detalle de sucursal (Resumen/Stock/Ventas), Ventas+3 tabs, Stock+3 tabs, Productos+3 tabs, Empleados+2 tabs, Configuración, y navegación a Dispositivos/Avisos/Auditoría — ver tarea de seguimiento en `TASKS.md`.
...
29. `202609220029_production_unit_outputs.sql`
30. `202609220030_manual_pricing.sql`
31. `202609230031_promotion_pack_fixed_total.sql`
32. `202609230032_product_unit_type_change_guard.sql`
...
7. `007_weight_discount_pack_mode.sql`
...
- **Venta POS `UNIT` end-to-end / directorio de categorías (2026-09-23, ronda 2 de esta sesión)**: `pnpm check` (typecheck+lint+test) **OK en los 7 proyectos del monorepo** — `packages/business-logic` 54 tests, `packages/sync` 8 tests (incluidos 2 nuevos: línea UNIT sin `weightGrams`, ticket mixto WEIGHT+UNIT), `apps/admin` 30 tests, `apps/pos` 32 tests. `pnpm --filter @carnicerias/pos build` (Vite) OK. `cargo test` (`apps/pos/src-tauri`) corrió contra SQLite real — **33/33 tests OK** (los 28 de la ronda 1 + 5 nuevos: `unit_sale_migration_preserves_preexisting_local_sale_history` —el que detectó el bug de `pragma foreign_keys` dentro de una transacción—, `unit_sale_charges_quantity_times_normal_price`, `unit_pack_sale_applies_exact_multiples`, `unit_pack_sale_applies_one_pack_plus_remainder_at_normal_price`, `unit_sale_below_pack_size_gets_no_discount`). Migración `202609230034_unit_sale_support.sql` y el test pgTAP nuevo (`supabase/tests/unit_sale_support.test.sql`, 16 aserciones, incluye el caso regresión de `cancel_sale` con línea UNIT) se revisaron manualmente línea por línea pero **no se ejecutaron contra Postgres real**: Docker Desktop no estuvo operativo en esta sesión. `database.types.ts` se actualizó a mano.
- **Promociones pack / corrección WEIGHT-UNIT / multicategoría (2026-09-23, ronda 1 de esta sesión)**: `pnpm check` (typecheck+lint+test) **OK en todo el monorepo**, confirmado con `pnpm check` corriendo cada paquete/app. `packages/business-logic` incluye 7 tests nuevos (`calculateWeightPackSalePricing`/`calculateUnitPackSalePricing`, incluido el caso límite de peso pesado por debajo del nominal). `apps/admin` incluye el build completo de `/admin/products` y `/admin/promotions` con la UI nueva (`pnpm --filter @carnicerias/admin build` corrido aparte, también OK). `apps/pos` incluye 2 tests nuevos de filtro multicategoría (`apps/pos/src/lib/catalog.test.ts`) y `pnpm --filter @carnicerias/pos build` (frontend Vite) OK. `cargo check` y `cargo test` (`apps/pos/src-tauri`) corrieron contra SQLite real — **28/28 tests OK**, incluidos 2 tests nuevos (`pack_sale_charges_fixed_total_regardless_of_weighed_grams`, `pack_sale_rejects_a_price_above_list_for_the_weighed_amount`) que ejercitan `insert_sale` de punta a punta contra una base SQLite en memoria. Migraciones `202609230031`–`202609230033` y los tests pgTAP nuevos (`supabase/tests/promotions_pack.test.sql`, `product_unit_type_guard.test.sql`, `product_multi_category.test.sql`) se revisaron manualmente línea por línea contra el schema y las funciones reales, pero **no se ejecutaron**: Docker Desktop no estuvo operativo en esta sesión (mismo bloqueo que sprints anteriores). `database.types.ts` se actualizó a mano.
- **Pricing manual / costo derivado / Desposte UNIT (2026-09-22, esta sesión)**: `pnpm typecheck` / `pnpm lint` / `pnpm test` / `pnpm build` OK en todo el monorepo (`packages/business-logic` incluye los 5 tests nuevos de asignación UNIT/mixta, `apps/admin` incluye el build completo de `/admin/products` y `/admin/production` con la UI nueva). `packages/database/src/database.types.ts` estaba de nuevo en UTF-16LE en el árbol de trabajo (mismo síntoma documentado más abajo en una sesión anterior); se reconvirtió a UTF-8 para poder editarlo. Migraciones `202609220029`/`202609220030` y los tests pgTAP nuevos/extendidos (`supabase/tests/production_batches.test.sql`, `supabase/tests/manual_pricing.test.sql`) se revisaron manualmente línea por línea contra el schema y las funciones reales, pero **no se ejecutaron**: Docker Desktop no estuvo operativo en esta sesión (mismo bloqueo que 024–028). `database.types.ts`/`database.rpc-null-overrides.ts` se actualizaron a mano.
