...
- Elegibilidad consistente en backend/POS/sync: `CASH`, `TRANSFER`, `OTHER` aplican; `DEBIT`, `CREDIT` no. Sin cambios: el orden de descuentos (lista → pago → promoción) y `complete_discounted_sale` no se tocaron.
...
`REQUIERE VERIFICACIÓN`: el repositorio está vinculado al proyecto Supabase, pero no se ejecutó `migration list --linked` en ninguna sesión reciente. No afirmar que ninguna migración desde la 022 en adelante (incluidas las 031–034 de este sprint) está aplicada al remoto hasta comprobarlo autenticadamente.
