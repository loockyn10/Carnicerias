...
- **Capas**: `packages/business-logic/src/pricing.ts` (las tres funciones de pricing ganan `cardSurchargeCents`; `calculateWeightPackSalePricing` pasa a ser invariante al medio de pago; `calculateUnitPackSalePricing` deja el recargo aplicarse sólo al remanente fuera del pack). `packages/types`/`packages/sync` (`cardSurchargeCents` nuevo, opcional, en `TicketLine`/`OfflineSaleItemPayload`). `apps/pos/src/App.tsx` (ticket y modal de vista previa muestran "Recargo tarjeta" en ámbar junto al ya existente "Descuento por pago", que en la práctica queda en 0 para toda venta nueva). `apps/pos/src-tauri/src/lib.rs` (`insert_sale` revalida `card_surcharge_cents` con la misma fórmula en los 4 casos WEIGHT/UNIT × pack/normal; `payment_method_receives_discount` se dejó intacto, sólo se invirtió cómo se usa su resultado). Admin (`pricing-settings-modal.tsx`: "RECARGO POR TARJETA" en vez de "DESCUENTO POR PAGO"; `set_cash_discount`'s mensaje de error actualizado).
- **Online y offline**: Postgres (`complete_discounted_sale`/`sync_offline_sale`, migración nueva `202609240035_card_surcharge_pricing.sql`) y Rust/SQLite (`insert_sale`, migración SQLite `010_card_surcharge_pricing.sql`) implementan la misma fórmula, con el mismo válido replicado — el offline puede rechazar un total manipulado igual que antes.
...
- **PACK_FIXED_TOTAL, ambigüedad resuelta conservadoramente (ver D-044)**: el total fijo de un pack (D-039) es también invariante al medio de pago — nunca lleva recargo tarjeta, igual que nunca varía con el peso/cantidad real. Sólo el remanente de un pack `UNIT` (unidades sueltas fuera del último pack completo) es una venta normal y sí lleva recargo. Es la resolución explícitamente reportada como decisión, no una regla inventada para hacer pasar tests.
- **REQUIERE VERIFICACIÓN**: Docker Desktop no estuvo operativo en esta sesión; `202609240035_card_surcharge_pricing.sql` y el pgTAP nuevo (`supabase/tests/card_surcharge_pricing.test.sql`, 24 aserciones) se revisaron manualmente línea por línea pero **no corrieron** contra Postgres real. Sí corrieron contra un motor real: `cargo test` (Rust/SQLite, 35/35 OK, incluidos 4 tests nuevos) y `pnpm check` (typecheck+lint+test) en todo el monorepo, además de `pnpm --filter @carnicerias/pos build` y `pnpm --filter @carnicerias/admin build`, todos verdes. `packages/database/src/database.types.ts` se actualizó a mano.
