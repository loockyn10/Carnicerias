...
- Admin typecheck/build: **actualmente en rojo**, por una razón preexistente y no relacionada con este sprint (ver "Materias primas, sucursal productiva y Distribución" arriba): `exactOptionalPropertyTypes` (activo en `tsconfig.base.json` desde el commit inicial) rechaza `null` explícito en varios RPC ya existentes (`saveCategoryAction`, `setPriceAction`, `recordPurchaseAction`, `recordReplenishmentFormAction`, `recordWasteAction`, `recordAdjustmentAction`, `manageMemberAction`, `createPosEmployeeAction`/`updatePosEmployeeAction`) y en las páginas `/admin/analytics`, `/admin/employees`, `/admin/settlements`, `/admin/timekeeping`. Esta entrada de "Validación actual" decía previamente "OK"; esa afirmación no se sostiene contra el estado real del código y se corrige aquí en vez de mantenerla. El código de este sprint (`/admin/production`, `/admin/transfers`, `/admin/products`, `packages/database`, `packages/types`) typechequea y lintea limpio de forma aislada (`pnpm --filter @carnicerias/admin typecheck`/`lint` no reportan ningún error en los archivos que este sprint modifica o agrega; los que quedan son 100% preexistentes, confirmados línea por línea). No se intentó una corrección global porque al menos un caso (`setPriceAction`) usa `null` con un significado propio distinto de omitir el argumento, y una corrección mecánica sin revisar cada RPC arriesgaría precios/reposición/empleados. Ver tarea de seguimiento en `TASKS.md`.
- Admin lint: limpio salvo el mismo hallazgo preexistente en `/admin/employees` (`@typescript-eslint/no-unnecessary-condition`), no tocado por este sprint.
- POS: sin cambios este sprint (Desposte/materias primas/transferencias son exclusivamente Admin, ver D-031); no se re-ejecutó su build.
