...
`PACK_FIXED_TOTAL` (D-039) sigue invariante al peso/cantidad real, pero no al medio de pago (corregido 2026-09-24, tras una primera implementación que sí lo dejaba invariante — ver el historial en D-044): el recargo por tarjeta se aplica al total del pack completo sin excepción ("Vacío 2kg/$18.000" con tarjeta = $19.800), y para un pack `UNIT` con remanente se aplica al total comercial completo (packs + remanente juntos), nunca sólo al remanente. Bug encontrado y corregido en la misma migración: `get_pos_commercial_config` había perdido la clave `cashDiscountBps` de su jsonb en un `CREATE OR REPLACE` anterior (202609230031).
