...
interface ProductManageModalProps {
  ...
  price: { cents: number; branchId: string | null } | null;
  promotion: { id: string; label: string } | null;
  categories: { id: string; name: string }[];
  branches: { id: string; name: string }[];
}

export function ProductManageModal({ product, price, promotion, categories, branches }: ProductManageModalProps) {
  ...
  const unitSuffix = product.unitType === "WEIGHT" ? "/kg" : "/unidad";
  const priceLabel = product.unitType === "WEIGHT" ? "Precio por kg" : "Precio por unidad";
    ...
    {open ? <div className="fixed inset-0 z-50 grid place-items-center bg-stone-950/30 p-4" onMouseDown={(event) => { if (event.currentTarget === event.target && !pending) setOpen(false); }}>
      <section aria-labelledby={`manage-product-${product.id}`} aria-modal="true" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-white p-5 text-left shadow-xl" role="dialog">
        ...
        <form action={action} className="mt-5 grid gap-4" ref={formRef}>
          ...
          <input name="current_price_cents" type="hidden" value={price?.cents ?? ""} /><input name="current_branch_id" type="hidden" value={price?.branchId ?? ""} />
          ...
          <div className="grid gap-3 sm:grid-cols-2"><label className="grid gap-1 text-sm font-medium">Ámbito del precio<select className={input} defaultValue={price?.branchId ?? ""} name="branch_id"><option value="">Global</option>{branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></label><label className="grid gap-1 text-sm font-medium">{priceLabel}<span className="flex items-center gap-2"><input className={`${input} min-w-0 flex-1`} defaultValue={price ? (price.cents / 100).toFixed(2) : ""} min="0.01" name="price" step="0.01" type="number" /><span className="text-stone-500">{unitSuffix}</span></span></label></div>
