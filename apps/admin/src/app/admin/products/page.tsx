...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [categoriesResult, productsResult, categoryAssignmentsResult, unitTypeHistoryResult, pricesResult, discountsResult, costsResult, cashResult] = await Promise.all([
    ...
    perf.measure("discounts", commercial.from("product_weight_discounts").select("id, product_id, branch_id, promotion_mode, minimum_grams, discount_type, discount_value, pack_quantity_grams, pack_quantity_units, pack_price_cents, active, valid_from, valid_until").eq("organization_id", context.organizationId).order("valid_from", { ascending: false })),
