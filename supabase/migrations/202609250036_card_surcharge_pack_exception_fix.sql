...
-- D-044 follow-up: the remote project already had 202609240035 applied (confirmed via
-- `supabase migration list --linked`), but with the CONTENT it had at push time — the initial,
-- more conservative reading where PACK_FIXED_TOTAL was payment-method-invariant (no surcharge on
-- packs at all: card_surcharge forced to 0 for a WEIGHT pack, and only the remainder surcharged
-- for a UNIT pack). That reading was corrected the same day, in place, in 202609240035's local
-- file, per the product owner's explicit clarification: "TODO lo que se pague con tarjeta lleva
-- el porcentaje de recargo configurado. No hay excepción para promociones ni packs." Because
-- 202609240035 was already applied remotely by the time that correction was made locally, editing
-- it in place (as documented in docs/DECISIONS.md D-044) only updated the LOCAL migration history
-- — `supabase db push` treats an already-applied version as done and will not re-run it. This
-- migration re-applies the corrected function bodies as a new, incremental step so the remote
-- database actually gets the correction. No schema/DDL changes are needed here: the columns this
-- introduced (sale_items.card_surcharge_cents, etc.) are already live on remote from the original
-- push. Only complete_discounted_sale and sync_offline_sale changed their PACK_FIXED_TOTAL branch;
-- get_pos_commercial_config and set_cash_discount already match between local and remote.

-- complete_discounted_sale (online checkout): same signature as 202609230034/202609240035. Order
-- is list -> promotion/pack -> card surcharge for every branch, no exception (D-044, corrected).
...
-- sync_offline_sale (offline push): same signature as 202609230034/202609240035, mirroring the
-- Rust insert_sale validation the device already ran. Same D-044 correction as
